...
import { parseChatProfile, parseProfileSelection, summarizeChatProfile } from "@/lib/profile";
...
export interface FollowUpSuggestionsPayload {
  profile: string;
  question: string;
  toneId: ToneId;
  tools: ToolKey[];
...
export function buildToolAnalysisPrompt(payload: {
  ...
  return [
    ...
    "이번 응답은 최종 상담이 아니라 현재 도구 단계의 짧은 요약이다.",
    '반드시 한국어 JSON 객체 하나만 반환한다. 형식은 {"summary":"..."} 뿐이다.',
    "summary는 2~3문장으로만 작성한다.",
    "아래 계산 결과는 서버가 확정한 값이므로 다시 계산하거나 수정하지 말고, 오직 해석에만 사용한다.",
    "행성 위치, 궁, 기둥, 카드, 배지 이름을 바꾸거나 새로 만들지 말라.",
    ...
    `지금은 ${tool.label} 기준으로만 위 계산 결과를 해석해라.`,
  ].join("\n");
}
...
export function buildLumoChatPrompt(payload: ChatRequestPayload): string {
  ...
  const profileBlock = selectedProfiles.length
    ? selectedProfiles
        .map((profile, index) => {
          const fields = parseChatProfile(profile);
          const displayName = fields.name.trim() || `프로필 ${index + 1}`;

          return `- ${displayName}: ${summarizeChatProfile(fields)}`;
        })
        .join("\n")
    : `- ${payload.profile}`;
  const historyBlock = payload.messages.length
    ? payload.messages
        ...
        .join("\n\n")
    : "이전 대화 없음";
  const activeToolBlock = activeToolMessages.length
    ? activeToolMessages.map((message) => message.content).join("\n\n")
    : "현재 답변에 적용할 도구 분석 없음";
...
export function buildFollowUpSuggestionsPrompt(payload: FollowUpSuggestionsPayload): string {
  ...
  const profileBlock = selectedProfiles.length
    ? selectedProfiles
        ...
        .join("\n")
    : `- ${payload.profile}`;
  const historyBlock = payload.messages
    .slice(-10)
    .map((message) => {
      if (message.role === "assistant") {
        return `루모 AI: ${message.content}`;
  ...
  return [
    "너는 루모 AI의 후속 질문 제안 생성기다.",
    '반드시 한국어 JSON 객체 하나만 반환한다. 형식은 {"suggestions":["...","...","..."]} 뿐이다.',
    "suggestions는 최대 3개다.",
    "각 질문은 짧고 바로 눌러 전송할 수 있게 자연스러운 1문장이어야 한다.",
    "서로 중복되면 안 되고, 방금 답변을 더 깊게 파는 방향이어야 한다.",
    "번호, 불릿, 설명문, 이모지, 따옴표 바깥 텍스트는 금지한다.",
    `현재 톤: ${tone.label} (${tone.guide})`,
    "출생 정보:",
    profileBlock,
    `이번 사용자 질문: ${payload.question}`,
    "최근 대화:",
